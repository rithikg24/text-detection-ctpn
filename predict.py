...
def get_text_boxes(image, display=True, prob_thresh=0.3):
    h, w = image.shape[:2]
    ...
    image = torch.from_numpy(image.transpose(2, 0, 1)).unsqueeze(0).float().to(device) # Making it into a pt tensor and adding extra dim for batch inputs
    
    with torch.no_grad():
        cls, regr = model(image)   # return classification score(probabilities) and reg values
